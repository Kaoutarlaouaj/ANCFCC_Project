...
if(isset($_GET['id'])) {
    // Récupérer l'ID du don à mettre à jour
    $id = $_GET['id'];
  
    // Inclure le fichier de connexion à la base de données
    include('connexion.php');
  
    // Récupérer les données du don à partir de la base de données
    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        // Rediriger si le don n'existe pas
        echo "<script>
...
if(isset($_POST['update'])) {
    ...
    // Requête SQL pour mettre à jour les données du don
    $updateQuery = "UPDATE donations SET category = '$newCategorie', created_at = '$newDate', amount = '$newMontant' WHERE id = $id";
...
<form method="post" style="margin-left:216px;">
    <div class="form-group">
        <label for="newCategorie">New Categorie:</label>
        <select class="form-control" id="newCategorie" name="newCategorie" required>
            <option value="education" <?php if($categorie == 'education') echo 'selected'; ?>>Education</option>
            <option value="health" <?php if($categorie == 'health') echo 'selected'; ?>>Health</option>
            <option value="shelter" <?php if($categorie == 'shelter') echo 'selected'; ?>>Shelter</option>
            <option value="nutrition" <?php if($categorie == 'nutrition') echo 'selected'; ?>>Nutrition</option>
            <option value="safety" <?php if($categorie == 'safety') echo 'selected'; ?>>Safety</option>
            <option value="recreation" <?php if($categorie == 'recreation') echo 'selected'; ?>>Recreation</option>
        </select>
    </div>
    <div class="form-group">
        <label for="newDate">New Date:</label>
        <input type="date" class="form-control" id="newDate" name="newDate" value="<?php echo $date; ?>" required>
    </div>
    <div class="form-group">
        <label for="newMontant">New Amount:</label>
        <input type="number" class="form-control" id="newMontant" name="newMontant" value="<?php echo $montant; ?>" required>
    </div>
