...
?>
<div class="modal fade" id="chatbotModal" tabindex="-1" role="dialog" aria-labelledby="chatbotModalLabelNotif" aria-hidden="true">
...
<button class="btn btn-primary" data-toggle="modal" data-target="#chatbotModal">Open Chatbot</button>
      ...
      <div class="modal-body">
        ...
        <?php //foreach ($messages as $message): ?>
          <div class="message">
            ...
            <div class="time"><?php //echo $message['time_sent']; ?></div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (password_verify($oldPassword, $user["password"])) {
        // Ancien mot de passe correct, mettre à jour les informations de l'utilisateur
        $hashedPassword = password_hash($newPassword, PASSWORD_DEFAULT);

        $updateSql = "UPDATE users SET name = '$newName', email = '$newEmail', password = '$hashedPassword' WHERE id = 1"; // Remplacez "1" par l'ID de l'utilisateur souhaité
        $updateResult = mysqli_query($conn, $updateSql);

        if ($updateResult) {
            echo "Informations mises à jour avec succès.";
            // Rediriger l'utilisateur vers la page de profil ou afficher un message de succès
        } else {
            echo "Une erreur s'est produite lors de la mise à jour des informations.";
            // Afficher un message d'erreur approprié ou rediriger l'utilisateur vers une page d'erreur
        }
    } else {
        echo "Ancien mot de passe incorrect.";
        // Afficher un message d'erreur indiquant que l'ancien mot de passe est incorrect
    }
}
...
// Affichage du formulaire d'édition des informations de l'utilisateur
?>
<!DOCTYPE html>
<html>
<head>
